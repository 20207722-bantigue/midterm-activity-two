...
const Register = ({ addStudent }) => {
  ...
  const validate = (field, value) => {
    ...
    if (field === "course" && !value.trim()) error = "Course is required";
  ...
  const isFormValid = () =>
    ...
    !errors.name && !errors.email && !errors.course;
    ...
    <div style={{ maxWidth: "500px", margin: "50px auto", padding: "20px", fontFamily: "Arial", backgroundColor: "#f9f9f9", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: "16px" }}>
          <label>Name:</label>
          <input type="text" name="name" value={formData.name} onChange={handleChange} style={{ width: "100%", padding: "8px", marginTop: "4px" }} />
        ...
        <div style={{ marginBottom: "16px" }}>
          <label>Course:</label>
          <input type="text" name="course" value={formData.course} onChange={handleChange} style={{ width: "100%", padding: "8px", marginTop: "4px" }} />
          {errors.course && <span style={{ color: "red", fontSize: "14px" }}>{errors.course}</span>}
        </div>
        <button type="submit" disabled={!isFormValid()} style={{ width: "100%", padding: "10px", backgroundColor: isFormValid() ? "#007bff" : "#ccc", color: "#fff", border: "none", borderRadius: "4px", cursor: isFormValid() ? "pointer" : "not-allowed" }}>
          Submit
        </button>
      </form>
    </div>
